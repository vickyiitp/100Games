...
interface DecisionScenarioGameProps {
  level: number;
  onGameComplete: (score: number) => void;
  game: Game;
}

const DecisionScenarioGame: React.FC<DecisionScenarioGameProps> = ({ level, onGameComplete, game }) => {
  const [totalScore, setTotalScore] = useState(0);
  
  const initialScenarioId = useMemo(() => {
    return (game.id % 2 === 1) ? 1 : 4;
  }, [game.id]);
...
export default DecisionScenarioGame;

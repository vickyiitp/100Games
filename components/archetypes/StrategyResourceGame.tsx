...
interface StrategyResourceGameProps {
  level: number;
  onGameComplete: (score: number) => void;
  game: Game;
}

const StrategyResourceGame: React.FC<StrategyResourceGameProps> = ({ level, onGameComplete }) => {
    ...
    useEffect(() => {
        if (day > totalDays) {
            ...
            const score = Math.floor(finalValue - 500);
    ...
    useEffect(() => {
        if (cash + shares * price >= target) {
            const finalValue = cash + shares * price;
            const score = Math.floor(finalValue - 500) + (totalDays - day) * 10 * level;
            onGameComplete(score);
        }
...
export default StrategyResourceGame;
